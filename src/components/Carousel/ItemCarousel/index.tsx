...
const ICarousel: React.FC<TICarousel> = ({ bgColor, widthHeight }) => {
    ...
    const projectsMap: any = api.JavaScript.map((projects: IProjects) => {
        return (
            <CardItemCarousel style={{ backgroundColor: `${bgColor ?? ''}`, width: `${widthHeight ?? '60rem'}`, height: `${widthHeight ?? '60rem'}` }} key={projects.id} className={api.JavaScript[0] === projects ? "carouselItem currentItem" : "carouselItem"}>
                <Image style={{
